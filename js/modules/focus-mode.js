/* TrueVoice: monotonic clocks. Hidden tabs pause practice; they never finish it silently. */
const atlasClocks=new Set();
class PracticeClock {
  constructor(seconds,onTick=()=>{},onComplete=()=>{}){
    this.total=Math.max(1,Number(seconds)||300);this.remaining=this.total;this.running=false;this.finished=false;
    this.onTick=onTick;this.onComplete=onComplete;this.frame=null;atlasClocks.add(this);
  }
...
class FocusPracticeManager {
  ...
  startSession(lesson,stepIndex=0,onComplete){
    if(!this.modalEl||!lesson?.practice?.length)return;
    this.returnFocus=document.activeElement;this.lesson=lesson;this.onCompleteCallback=onComplete;
    this.modalEl.classList.add('open');this.loadStep(stepIndex);
    this.modalEl.querySelector('#focus-close-btn')?.focus();
  }
  loadStep(idx){
    this.clock?.destroy();this.currentStepIdx=Math.max(0,Math.min(this.lesson.practice.length-1,idx));
    const step=this.lesson.practice[this.currentStepIdx];
    this.clock=new PracticeClock(window.atlasSeconds(step.d),(seconds,running)=>{
      this.remainingSeconds=seconds;this.isRunning=running;this.updateTimerDisplay();this.updatePlayBtn(running);
    },()=>this.stepCompleted());
    this.remainingSeconds=this.clock.total;this.isRunning=false;this.updateDOM();
    this.startTimer();
  }
  ...
  stepCompleted(){
    this.isRunning=false;this.remainingSeconds=0;this.updateDOM();window.audioEngine?.playCompletionChime();
    this.onCompleteCallback?.(this.currentStepIdx);
    const b=this.modalEl?.querySelector('#focus-play-btn');if(b){b.textContent='↻ Повторити крок';b.setAttribute('aria-pressed','false');}
    const m=this.modalEl?.querySelector('.focus-step-meta');if(m)m.textContent=this.currentStepIdx===this.lesson.practice.length-1?'ПРАКТИКУ ЗАВЕРШЕНО ✓':'КРОК ЗАВЕРШЕНО ✓ · ПЕРЕЙДИ ДО НАСТУПНОГО';
  }
  ...
  close(){this.clock?.destroy();this.clock=null;this.isRunning=false;this.modalEl?.classList.remove('open');this.returnFocus?.isConnected&&this.returnFocus.focus({preventScroll:true});}
  ...
  updateDOM(){
    if(!this.modalEl||!this.lesson)return;const step=this.lesson.practice[this.currentStepIdx];
    const set=(q,t)=>{const e=this.modalEl.querySelector(q);if(e)e.textContent=t;};
    set('.focus-step-meta',`КРОК ${this.currentStepIdx+1} З ${this.lesson.practice.length} · ТИЖДЕНЬ ${this.lesson.w}`);
    set('.focus-step-title',step.n);set('.focus-step-desc',step.t);this.updateTimerDisplay();this.updatePlayBtn(this.isRunning);
    this.modalEl.querySelector('#focus-prev-btn').disabled=this.currentStepIdx===0;
    this.modalEl.querySelector('#focus-next-btn').disabled=this.currentStepIdx===this.lesson.practice.length-1;
  }
}
