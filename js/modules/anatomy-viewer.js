...
const SOURCES=[['NIDCD · How voice is produced','https://www.nidcd.nih.gov/health/taking-care-your-voice'],['NHLBI · How the lungs work','https://www.nhlbi.nih.gov/health/lungs/breathing']];
...
export class AnatomyViewer {
 ...
 updatePlate(){
  ...
  q('.atlas-zone-list').innerHTML=p.zones.map((z,i)=>`<button type="button" data-zone="${i}" aria-pressed="${i===0}"><span class="mono">0${i+1}</span>${escapeHTML(z[1])}</button>`).join('');
  q('.atlas-schematic').innerHTML=diagram(this.view);
  this.root.querySelectorAll('[data-plate]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.plate===this.view)));
  this.selectZone(0);this.updateMode();
 }
 ...
 transform(){
  const bx=this.stage.clientWidth*(this.zoom-1)/2,by=this.stage.clientHeight*(this.zoom-1)/2;this.x=Math.max(-bx,Math.min(bx,this.x));this.y=Math.max(-by,Math.min(by,this.y));
  this.root.querySelector('.atlas-transform').style.transform=`translate(${this.x}px,${this.y}px) scale(${this.zoom})`;this.root.querySelector('.atlas-scale').textContent=`${Math.round(this.zoom*100)}%`;
 ...
 bind(){
  ...
  this.root.addEventListener('click',e=>{
    ...
    else if(b.hasAttribute('data-expand')){this.root.classList.toggle('atlas-expanded');b.setAttribute('aria-pressed',String(this.root.classList.contains('atlas-expanded')));this.resetZoom();}
    else if(b.hasAttribute('data-pacer-toggle'))window.breathPacer.toggle(this.root);
    else if(b.hasAttribute('data-pacer-reset'))window.breathPacer.reset();
  },opts);
 ...
 destroy(){window.breathPacer.stop();if(window.breathPacer.activeEl===this.root)window.breathPacer.activeEl=null;this.abort?.abort();this.observer?.disconnect();}
}
