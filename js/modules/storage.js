/* TrueVoice Atlas: version-compatible, validated local progress. No account or server storage. */
...
window.atlasSeconds = function (value) {
  const s = String(value || '').replace(',', '.');
  const min = s.match(/(\d+(?:\.\d+)?)\s*хв/i), sec = s.match(/(\d+(?:\.\d+)?)\s*(?:сек|с\b)/i);
  return Math.max(1, Math.round((min ? Number(min[1]) * 60 : 0) + (sec ? Number(sec[1]) : 0)) || 300);
};
class AtlasStorageManager {
  constructor() {
    this.state = this.defaults(); this.available = true; this.load();
  }
  defaults() {
    return {idx:0, mode:'student', theme:'default', showSpine:true, showStarfield:true, soundFX:true, volume:0.6,
      done:{}, notes:{}, bookmarks:[], lastVisit:new Date().toISOString(), streak:1};
  }
  normalize(input, strict = false) {
    ...
    if (['default','ember','violet','teal','gold','crimson','cyan','ice'].includes(input.theme)) out.theme = input.theme;
    ...
    if (isRecord(input.done)) for (const [key, rec] of Object.entries(input.done)) {
      ...
      const max = lesson ? lesson.practice.length : 100;
      out.done[key] = {lesson:rec.lesson === true, steps:[...new Set(rec.steps.filter(n => Number.isInteger(n) && n >= 0 && n < max))]};
    }
    if (isRecord(input.notes)) for (const [key, value] of Object.entries(input.notes)) {
      if (lessonKeyOK(key) && typeof value === 'string') out.notes[key] = value.slice(0,100000);
      else if (strict) throw new Error('Пошкоджені нотатки.');
    }
    if (Array.isArray(input.bookmarks)) out.bookmarks = [...new Set(input.bookmarks.filter(k => typeof k === 'string' && lessonKeyOK(k)))];
    return out;
  }
  load() {
    try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) this.state = this.normalize(JSON.parse(raw)); this.calculateStreak(); }
    catch (e) { this.available = false; console.warn('Atlas: local state unavailable', e); }
  }
  save() {
    try { this.state.lastVisit = new Date().toISOString(); localStorage.setItem(STORAGE_KEY,JSON.stringify(this.state)); this.available = true; return true; }
    catch (e) { this.available = false; console.warn('Atlas: state not persisted',e); return false; }
  }
  ...
  setStepDone(l,i,done=true) {
    if (!Number.isInteger(i) || !l.practice[i]) return false;
    const r=this.getLessonRecord(l), set=new Set(r.steps); done ? set.add(i) : set.delete(i);
    r.steps=[...set].sort((a,b)=>a-b); r.lesson=r.steps.length===l.practice.length; this.save(); return done;
  }
  ...
  exportDataJSON() {
    const url=URL.createObjectURL(new Blob([JSON.stringify(this.state,null,2)],{type:'application/json'}));
    const a=document.createElement('a'); a.href=url; a.download=`truevoice-atlas-${new Date().toISOString().slice(0,10)}.json`; a.click(); setTimeout(()=>URL.revokeObjectURL(url),1000);
  }
  importDataJSON(text) {
    const previous=this.state;
    try { const next=this.normalize(JSON.parse(text),true); this.state=next; if (!this.save()) {this.state=previous;return false;} return true; }
    catch(e) { this.state=previous; return false; }
  }
