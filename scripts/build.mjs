...
await mkdir(path.join(out, 'assets', 'anatomy'), { recursive: true });
...
for (const name of ['tract', 'breath', 'larynx', 'body']) {
  const asset = pack[name];
  if (!asset || asset.width !== 1122 || asset.height !== 1402) throw new Error(`Invalid artwork metadata: ${name}`);
...
for (const match of [...html.matchAll(/(?:src|href)="((?:css|js)\/[^"?]+\.(?:js|css))"/g)]) {
  const version = hash(await readFile(path.join(root, match[1]))).slice(0, 12);
  html = html.replaceAll(`"${match[1]}"`, `"${match[1]}?v=${version}"`);
}
// Avoid a flash of the legacy layout while the app's dynamic viewer loads.
html = html.replace('</head>', '<link rel="stylesheet" href="css/atlas-upgrade.css?v=3.0.0"/>\n</head>');
