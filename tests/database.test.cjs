...
await db.exec("reset role;update tv_core.orders set status='approved' where id='o-a';update tv_core.entitlements set valid_from=now()-interval '1 day',valid_until=now()-interval '1 second' where order_id='o-a'");await claims(db,A,SA);
